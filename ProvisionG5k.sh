...
# if UI/X needed
apt-get --no-install-recommends install gnome-core gdm gnome-session-fallback firefox libwebkitgtk-1.0-0 tightvncserver xorg -y

# clear cached packages to save disk space
apt-get clean

# add jar and script to munin
cp tools/jmx2munin/jmx2munin-1.0.jar /usr/share/munin/jmx2munin.jar
cp tools/jmx2munin/jmx2munin.sh /usr/share/munin/plugins
